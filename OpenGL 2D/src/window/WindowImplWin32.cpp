...
namespace
{
    ...
    priv::WindowImplWin32*		fullscreenWindow = nullptr;

	static std::vector<std::string> droppedFiles;

	DWORD getStyle(Uint32 style)
	{
...
WindowImplWin32::WindowImplWin32(VideoMode mode, const std::string& title, Uint32 style, const ContextSettings& /*settings*/) :
m_handle          (NULL),
m_callback        (0),
m_cursor          (NULL),
m_icon            (NULL),
...
m_cursorTrapped   (false)
{
	...
	DWORD win32Style = getStyle(style);
    ...
    // Create the window
	m_handle = CreateWindow(className, Util::s2ws(title).c_str(), win32Style, left, top, width, height, NULL, NULL, GetModuleHandle(NULL), this);
	...
	setDragDropAllowed(false);

    // Increment window count
    windowCount++;
}

WindowImplWin32* WindowImplWin32::create(VideoMode mode, const std::string& title, Uint32 style, const ContextSettings& settings)
{
	return new WindowImplWin32(mode, title, style, settings);
}

WindowImplWin32* WindowImplWin32::create(WindowHandle handle)
{
	return new WindowImplWin32(handle);
}
...
bool WindowImplWin32::popEvent(Event& event, bool block)
{
...
void WindowImplWin32::pushEvent(const Event& event)
{
	m_events.push(event);
}
...
Vector2i WindowImplWin32::getSize() const
{
    RECT rect;
...
void WindowImplWin32::setSize(const Vector2i& size)
{
...
{
    SetWindowText(m_handle, Util::s2ws(title).c_str());
}
...
bool WindowImplWin32::switchToFullscreen(const VideoMode& mode)
{
	if (!isFullscreen())
	{
		m_lastDesktopSize = getSize();
...
bool WindowImplWin32::switchToWindowed(const VideoMode& mode, Uint32 style, bool useLastSizeAndPosition)
{
	if ((style & Style::Fullscreen) != 0)
	{
		return false;
	}

	if (VideoMode::getDesktopMode() == mode)
	{
		return false;
	}
	...
	SetWindowLong(m_handle, GWL_STYLE, win32Style);
	...
	if (useLastSizeAndPosition)
	{
		x = m_lastPosition.x;
		y = m_lastPosition.y;
		width = m_lastDesktopSize.x;
		height = m_lastDesktopSize.y;
	}
	...
	SetWindowPos(m_handle, NULL, x, y, width, height, SWP_NOZORDER);
...
bool WindowImplWin32::isFullscreen() const
{
	return fullscreenWindow != nullptr;
}
...
void WindowImplWin32::setDragDropAllowed(bool flag)
{
	m_dragDropAllowed = flag;
	DragAcceptFiles(m_handle, flag);
}

bool WindowImplWin32::isDragDropAllowed() const
{
	return m_dragDropAllowed;
}
    ...
    switch (message)
    {
		...
		case WM_DROPFILES: 
		{
			...
			for (int i = 0; i < droppedFileCount; ++i) 
			{
				...
				droppedFiles.push_back(std::string(fileName));
			}
			...
			Event event;
			event.type = Event::FileDrop;
			event.fileDrop.x = dropPoint.x;
			event.fileDrop.y = dropPoint.y;
			event.fileDrop.fileCount = droppedFileCount;
			event.fileDrop.files = &droppedFiles[0];
			pushEvent(event);
			break;
		}
    }
